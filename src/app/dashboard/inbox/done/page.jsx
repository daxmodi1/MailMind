...
export default function DonePage() {
  ...
  useEffect(() => {
    async function fetchDoneEmails() {
      try {
        const res = await fetch('/api/gmail?type=done');
  ...
  return (
    <div className="p-4 max-w-4xl mx-auto">
      <h1 className="text-2xl font-bold mb-4">Done Emails ({emails.length})</h1>

      {emails.length === 0 ? (
        <div className="text-center py-8 text-gray-500">
          No Done emails found.
        </div>
      ) : (
        <div className="space-y-2">
          {emails.map((email) => (
            <Link
              key={email.id}
              href={`/dashboard/inbox/done/${email.id}`}
